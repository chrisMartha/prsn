Param([string]$azureImport="C:\Program Files (x86)\Microsoft SDKs\Azure\PowerShell\ServiceManagement\Azure\Azure.psd1",
      ...
      [string]$serviceName="pems-dev",
      [string]$containerName="mydeployments",
      [string]$config=".\AzureDeploy\bin\Dev\app.publish\ServiceConfiguration.DEV.cscfg",
      [string]$package=".\AzureDeploy\bin\Dev\app.publish\AzureDeploy.cspkg",
      [string]$slot="Production",
      [string]$role1 = "PSoC.ManagementService",
      [string]$role2 = "PSoC.ManagementService.LicenseTimer",
	  [string]$wadConfigPath=".\diagnostics.DEV.wadcfgx")
...
try {
    ...
    if ($env:computername -eq "DEV-WIN01")
    {
        ...
        $config = "C:\Projects\Pearson\enterprise-management-service\AzureDeploy\bin\Dev\app.publish\ServiceConfiguration.DEV.cscfg"
		$wadConfigPath="C:\Projects\Pearson\enterprise-management-service\diagnostics.DEV.wadcfgx"
    }
