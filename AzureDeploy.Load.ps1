Param([string]$azureImport="C:\Program Files (x86)\Microsoft SDKs\Azure\PowerShell\ServiceManagement\Azure\Azure.psd1",
      ...
      [string]$storageAccount="pemsload",
	  [string]$storageAccountPrimaryAccessKey="G8xqcqKHxToxaq3mBAQt/RaiJTJfgSWSsQ985wg8HW81RQuilaNplKBCsKg8UPupQxjd7rtL9RCjewmXpnAkUA==",
      [string]$subscription="CCSoC Development",
      [string]$serviceName="pems-load",
      [string]$containerName="mydeployments",
      [string]$config=".\AzureDeploy\bin\Load\app.publish\ServiceConfiguration.Load.cscfg",
      [string]$package=".\AzureDeploy\bin\Load\app.publish\AzureDeploy.cspkg",
      [string]$slot="Production",
      [string]$role1 = "PSoC.ManagementService",
      [string]$role2 = "PSoC.ManagementService.LicenseTimer",
	  [string]$wadConfigPath=".\diagnostics.Load.wadcfgx")
...
try {
    ...
    if ($env:computername -eq "DEV-WIN01")
    {
        ...
        $config = "C:\Projects\Pearson\enterprise-management-service\AzureDeploy\bin\Load\app.publish\ServiceConfiguration.Load.cscfg"
		$wadConfigPath="C:\Projects\Pearson\enterprise-management-service\diagnostics.Load.wadcfgx"
    }
	...
	Write-Host "Enabled cloud servicce diagnostics extension on all roles at $slot deployment slot."
    Write-Host ""
